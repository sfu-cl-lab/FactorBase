...
public final class DataUtils2 {
    private static Map<Integer, DoubleMatrix2D> rememberSubmatricesMap
            = new HashMap<Integer, DoubleMatrix2D>();
     ...
     */
    public static DoubleMatrix2D subMatrix(ICovarianceMatrix cov, List<Node> variables, Node x, Node y, List<Node> z) {
        if (x == null) {
            throw new NullPointerException();
        }
...
//        DoubleMatrix2D _covMatrix = m;
        ...
        if (rememberSubmatricesMap.get(2 + z.size()) == null) {
            DoubleMatrix2D submatrix = new DenseDoubleMatrix2D(2 + z.size(), 2 + z.size());
            rememberSubmatricesMap.put(2 + z.size(), submatrix);
        }

        DoubleMatrix2D submatrix = rememberSubmatricesMap.get(2 + z.size());
        ...
        // Extract submatrix of correlation matrix using this index array.
//        DoubleMatrix2D submatrix = _covMatrix.viewSelection(indices, indices).copy();
