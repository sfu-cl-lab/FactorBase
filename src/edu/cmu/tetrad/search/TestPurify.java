...
//        System.out.println(_partition);
    ...
    public void test2() {
        Graph graph = new EdgeListGraph(GraphUtils.randomMim(5, 5, 5, 10, 10, 10));
//        Graph graph = GraphUtils.randomMim(10, 10, 5, 0, 0, 0);
...
//        System.out.println(purifiedClustering);
        ...
        for (int i = 0; i < purifiedClustering.size(); i++) {
            for (int j = 0; j < purifiedClustering.get(i).size(); j++) {
                clusters.addToCluster(i, purifiedClustering.get(i).get(j).getName());
            }
        }

        IndTestMimBuild test2 = new IndTestMimBuild(data, 0.001, clusters);
        test2.setAlgorithmType(IndTestMimBuild.MIMBUILD_GES_ABIC);

        MimBuild mimbuild = new MimBuild(test2, new Knowledge());
        mimbuild.setAlpha(0.001);
        Graph _graph = mimbuild.search();
