...
INSERT INTO `ADT_RNodes_False_WHERE_List` VALUES ('`b`','`b_star`.`diff(course0)`=`b_flat`.`diff(course0)`'),('`a`','`a_star`.`popularity(prof0)`=`a_flat`.`popularity(prof0)`'),('`b`','`b_star`.`rating(course0)`=`b_flat`.`rating(course0)`'),('`a`','`a_star`.`teachingability(prof0)`=`a_flat`.`teachingability(prof0)`'),('`a`','`a_star`.`intelligence(student0)`=`a_flat`.`intelligence(student0)`'),('`b`','`b_star`.`intelligence(student0)`=`b_flat`.`intelligence(student0)`'),('`a`','`a_star`.`ranking(student0)`=`a_flat`.`ranking(student0)`'),('`b`','`b_star`.`ranking(student0)`=`b_flat`.`ranking(student0)`');
